...
describe('HighLightButton Directive', () => {
  ...
  it('should set appropriate colors on both buttons', waitForAsync(() => {
    ...
    button1.nativeElement.dispatchEvent(new Event('click'));
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      expect(button1.nativeElement.style.backgroundColor).toBe('rgb(199, 200, 190)');
      expect(button2.nativeElement.style.backgroundColor).toBe('rgb(240, 240, 240)');
    });
  }));
});
